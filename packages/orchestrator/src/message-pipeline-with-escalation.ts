...
import { CompletionMessage, CompletionParams, CompletionResponse, LLMProvider, ProviderConfig, createLLMProvider } from '@meta-chat/llm';
...
interface MessagePipelineWithEscalationOptions {
  tenantId: string;
  channel: ChannelType;
  ...
  escalationConfig?: Partial<EscalationEngineConfig>;
...
interface CompletionOutcome {
  ...
  response?: CompletionResponse; // Full response for confidence analysis
}

const logger = createLogger('MessagePipelineWithEscalation');

/**
 * Enhanced message pipeline with confidence-based escalation
 *
 * This extends the standard MessagePipeline with intelligent human handoff
 * based on LLM confidence assessment rather than just keyword matching.
 */
export class MessagePipelineWithEscalation {
  ...
  private readonly enableConfidenceEscalation: boolean;

  constructor(private readonly options: MessagePipelineWithEscalationOptions) {
    ...
    this.enableConfidenceEscalation = options.enableConfidenceEscalation ?? true;

    // Initialize escalation engine with options config (can be overridden per-tenant)
    this.escalationEngine = new EscalationEngine(
      options.escalationConfig,
      this.eventManager
    );

    logger.info('MessagePipelineWithEscalation initialized', {
      tenantId: options.tenantId,
      channel: options.channel,
      enableConfidenceEscalation: this.enableConfidenceEscalation,
    });
  }

  async process(message: NormalizedMessage): Promise<void> {
    const correlationId = ensureCorrelationId();
    logger.info('Processing inbound message with escalation', {
      correlationId,
    ...
    // ENHANCEMENT: Update escalation engine config from tenant settings
    const tenantEscalationConfig = buildEscalationConfigFromTenant(config.settings);
    const isConfidenceEnabled = this.enableConfidenceEscalation && isConfidenceEscalationEnabled(config.settings);

    if (tenantEscalationConfig && isConfidenceEnabled) {
      this.escalationEngine.updateConfig(tenantEscalationConfig);
      logger.debug('Updated escalation config from tenant settings', {
        tenantId: this.options.tenantId,
        mode: config.settings.confidenceEscalation?.mode,
      });
    }
    ...
    // ENHANCEMENT: Check keyword-based handoff first (backward compatible)
    if (this.shouldTriggerHumanHandoff(config, message)) {
      await this.handleHumanHandoff(conversation, message, config, 'keyword');
      return;
    }
    ...
    // ENHANCEMENT: Run LLM with confidence prompting
    const completionOutcome = await this.runLLMWithConfidence(
      conversation.id,
      message,
      history,
      config,
    ...
    // ENHANCEMENT: Confidence-based escalation decision
    let finalContent = completionOutcome.content.trim();

    if (isConfidenceEnabled && completionOutcome.response) {
      const decision = await this.escalationEngine.decide(
        completionOutcome.response,
        {
          tenantId: this.options.tenantId,
          conversationId: conversation.id,
      ...
      logger.info('Escalation decision made', {
        ...
        shouldSendResponse: decision.shouldSendResponse,
        shouldNotifyHuman: decision.shouldNotifyHuman,
      });

      // Handle immediate escalation
      if (decision.action === EscalationAction.IMMEDIATE_ESCALATION) {
        await this.handleHumanHandoff(conversation, message, config, 'low_confidence', decision.analysis);
        return;
      }

      // Use modified response if available (with disclaimers)
      if (decision.modifiedResponse) {
        finalContent = decision.modifiedResponse;
      }
    ...
    logger.info('Completed message processing with escalation', {
      tenantId: this.options.tenantId,
      channel: this.options.channel,
      conversationId: conversation.id,
    });
   ...
   * ENHANCEMENT: Run LLM with confidence prompting
   */
  private async runLLMWithConfidence(
    ...
    // ENHANCEMENT: Augment system prompt with confidence assessment instructions
    // (isConfidenceEnabled is determined in process() method from tenant settings)
    const baseSystemPrompt = this.buildSystemPrompt(config, ragResults);
    const promptBuilder = this.escalationEngine.getPromptBuilder();
    const isEnabled = this.enableConfidenceEscalation && isConfidenceEscalationEnabled(config.settings);
    const systemPrompt = isEnabled
      ? promptBuilder.augmentSystemPrompt(baseSystemPrompt)
      : baseSystemPrompt;
    ...
    let latestContent = '';
    let latestResponse: CompletionResponse | undefined;
    ...
    while (iterations < this.maxToolIterations) {
      ...
      const { content, usage, toolCalls, response } = await this.streamCompletion(provider, params);
    ...
    return {
      ...
      usage: usageTotals,
      response: latestResponse, // Include full response for confidence analysis
    };
  }
  ...
  private async streamCompletion(provider: LLMProvider, params: CompletionParams): Promise<{
    ...
    for await (const chunk of stream) {
      ...
      if (chunk.delta.model) {
        model = chunk.delta.model;
      }
    ...
    // Construct full response for confidence analysis
    ...
    return { content, usage, toolCalls, response };
  }

  private mergeToolCalls(toolStates: Map<string, ToolCallState>, chunk: any): void {
  ...
  private async handleHumanHandoff(
    ...
    await this.emitEvent(EventType.HUMAN_HANDOFF_REQUESTED, {
      ...
      channel: this.options.channel,
      reason,
      keywords: reason === 'keyword' ? config.settings.humanHandoffKeywords : undefined,
  ...
  /**
   * Get the escalation engine (for configuration updates)
   */
